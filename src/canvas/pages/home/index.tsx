...
import { fetchPrompts, type Prompt } from "@canvas/services/api/prompts";
...
function Highlighter({ action, color, children }: { action: "highlight" | "underline"; color: string; children?: ReactNode }) {
    return (
        <span className="relative inline-block px-1">
            ...
            <span className="relative font-medium text-zinc-800 dark:text-zinc-200">{children}</span>
        </span>
    );
}
...
export default function IndexPage() {
    ...
    return (
        <main className="relative h-full overflow-y-auto bg-background bg-[radial-gradient(#e5e7eb_1px,transparent_1px)] [background-size:16px_16px] text-zinc-950 dark:bg-[radial-gradient(rgba(245,245,244,.18)_1px,transparent_1px)] dark:text-zinc-100">
            <section className="relative mx-auto min-h-[calc(100vh-4rem)] max-w-7xl overflow-hidden px-6">
                <div className="pointer-events-none absolute left-[15%] top-24 size-20 rounded-full border border-dashed border-zinc-200 dark:border-zinc-800" />
                <div className="pointer-events-none absolute right-[23%] top-[48%] size-20 rounded-full border border-dashed border-zinc-200 dark:border-zinc-800" />
                ...
                <div className="relative flex min-h-[620px] flex-col items-center justify-center pt-10 text-center">
                    ...
                    <p className="mt-8 max-w-3xl text-balance text-lg leading-8 text-zinc-500 dark:text-zinc-400">
                ...
                <section className="relative mx-auto mb-20 max-w-6xl border-t border-zinc-200 pt-12 dark:border-zinc-800">
                    <div className="mb-8 grid gap-4 md:grid-cols-[1fr_auto_1fr] md:items-start">
                        <div />
                        <div className="max-w-2xl text-center">
                            <h2 className="text-3xl font-semibold text-zinc-950 dark:text-zinc-100">{t("home.showcaseTitle")}</h2>
                            <p className="mt-3 text-base leading-7 text-zinc-500 dark:text-zinc-400">{t("home.showcaseDescription")}</p>
                        </div>
                    ...
                    <div className="grid auto-rows-[210px] gap-4 md:grid-cols-4">
                        {promptShowcase.map((item, index) => (
                            <button
                                ...
                                className={cn(
                                    "group relative cursor-pointer overflow-hidden border border-zinc-200 bg-zinc-100 text-left dark:border-zinc-800 dark:bg-zinc-900",
