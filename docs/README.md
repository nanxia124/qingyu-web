...
5. **<span style="font-size:18px;color:#fff">写进规范时，如果发现文档里以前写的老规矩跟这次新做法对不上、打架，就把老规矩里冲突的那几句改掉，以这次新做法为准——不是整段删，只改冲突的那几句，别让文档里同时留着两条互相矛盾的说法。</span>**<br>
   <span style="display:inline-block;font-size:12px;color:#666;transform:scale(0.35);transform-origin:left top;line-height:1">写入规范时若新旧条目存在冲突，以本次验证通过的新内容为准，覆盖/删除旧条目中与新内容矛盾的片段；非冲突的历史内容保留。"只增不删"总原则在此处对"冲突部分"放行。</span>
