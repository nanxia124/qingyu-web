import { isSiteTool, SITE_TOOL_LABELS } from "@canvas/lib/agent/agent-site-tools";
import i18n from "@canvas/i18n";
import { summarizeCanvasAgentOps, type CanvasAgentOp } from "@canvas/lib/canvas/canvas-agent-ops";
...
import { resolveAgentMessageAssetUrl } from "@canvas/services/api/canvas-agent";
import { useAgentStore, type AgentAttachment, type AgentChatItem, type AgentEventLog, type AgentMessageAttachment, type AgentTokenUsage } from "@canvas/stores/use-agent-store";
...
export function formatAgentEvent(event: AgentEventPayload): Omit<AgentChatItem, "id"> | null {
    ...
    if (event.type === "item.completed" && item?.type === "agent_message") return { role: "assistant", title: "Agent", text: stringText(item.text) };
    return null;
}
...
export function bindPendingTurnMessages(messages: AgentChatItem[], threadId: string, turnId: string) {
    ...
    for (let i = messages.length - 1; i >= 0; i--) {
        if (messages[i].role === "user" && messages[i].threadId === threadId && !messages[i].turnId) {
            index = i;
            break;
        }
    }
