...
import { useAgentStore, type AgentCanvasReference, type AgentPendingApproval, type AgentSkillReference } from "@canvas/stores/use-agent-store";
            ...
            >
                {isUser ? (
                    ...
                    <div className="group relative">
                        ...
                        {!expanded && !item.streamId && item.text.length > 200 ? (
                            <button type="button" className="mt-1 text-xs font-medium" style={{ color: theme.node.activeStroke }} onClick={() => setExpanded(true)}>
                                展开全文
                            </button>
                        ) : expanded && !item.streamId && item.text.length > 200 ? (
                            <button type="button" className="mt-1 text-xs font-medium" style={{ color: theme.node.muted }} onClick={() => setExpanded(false)}>
                                收起
                            </button>
                        ) : null}
                        {!item.streamId ? (
...
export function AgentPendingToolCard({ summary, detail, theme, onReject, onApprove }: { summary: string; detail?: unknown; theme: (typeof canvasThemes)[keyof typeof canvasThemes]; onReject?: () => void; onApprove?: () => void }) {
    ...
    return (
        <div className="min-w-0 rounded-xl border px-3 py-3" style={{ background: "rgba(80,81,248,.08)", color: theme.node.text }}>
            <details className="group">
                <summary className={`list-none ${view ? "cursor-pointer" : "cursor-default"}`} onClick={(event) => { if (!view) event.preventDefault(); }}>
                    <div className="flex min-w-0 items-center gap-2 text-sm font-medium leading-5">
                        <CircleAlert className="size-4 shrink-0" style={{ color: "#5051F8" }} />
    ...
    return (
        <div className="min-w-0 rounded-xl border px-3 py-3" style={{ background: "rgba(80,81,248,.08)", color: theme.node.text }}>
            <div className="flex items-start gap-2.5">
                <ShieldAlert className="mt-0.5 size-4 shrink-0" style={{ color: "#5051F8" }} />
