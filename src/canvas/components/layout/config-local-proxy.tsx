...
import { testLocalProxy } from "@canvas/services/api/local-proxy";
import { DEFAULT_LOCAL_PROXY_URL, LOCAL_PROXY_PACKAGE, normalizeLocalProxyUrl, useConfigStore } from "@canvas/stores/use-config-store";
            ...
            <section className="rounded-lg border border-zinc-200 p-3 dark:border-zinc-800">
                <div className="flex flex-wrap items-start justify-between gap-3">
                    <div>
                        ...
                        <div className="mt-1 text-xs text-zinc-500">{t("config.proxy.description")}</div>
                    </div>
                    <Switch checked={config.proxyEnabled} onChange={(checked) => updateConfig("proxyEnabled", checked)} />
                </div>
                {config.proxyEnabled ? (
                    <>
                        <div className="mt-3 rounded-md bg-zinc-100 px-3 py-2 dark:bg-zinc-900">
                            <div className="mb-1 text-xs text-zinc-500">{t("config.proxy.startHint")}</div>
                        ...
                        <div className="mt-3 text-xs text-zinc-500">{t("config.proxy.channelHint")}</div>
                    </>
                ) : null}
            </section>
        </Form>
