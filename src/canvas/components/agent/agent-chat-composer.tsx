...
export function AgentChatComposer({
    ...
    const addSelectedCanvasReferences = () => {
        ...
        const allRefs = buildCanvasResourceReferences(snapshot.nodes || []);
        const selectedRefs = allRefs.filter((r) => snapshot.selectedNodeIds.includes(r.nodeId));
        if (!selectedRefs.length) return;
        const current = useAgentStore.getState().canvasReferences;
        const existingIds = new Set(current.map((r) => r.nodeId));
        const newRefs = selectedRefs.filter((r) => !existingIds.has(r.nodeId));
        if (!newRefs.length) return;
        useAgentStore.getState().setAgentState({ canvasReferences: [...current, ...newRefs] });
        const markers = newRefs.map((r) => `@${r.label || r.title || r.kind}`).join(" ");
        onPromptChange(prompt ? `${prompt} ${markers} ` : `${markers} `);
    };
