...
function getVideoConfig() {
    ...
    return {
        ...
        secondsRange: (() => {
            // 时长范围随当前视频模型收窄（固定档模型回传可选档位），不再给统一的 4–30 秒。
            const rule = getVideoDurationRule(modelOptionName(model));
            return rule.presets
                ? { min: rule.presets[0], max: rule.presets[rule.presets.length - 1], presets: rule.presets }
...
function runVideoWorkbench(input: SiteToolInput, navigate: NavigateFunction) {
    const configStore = useConfigStore.getState();
    const applied: Record<string, unknown> = {};
    let finalModel = configStore.config.videoModel || configStore.config.model;
    if (typeof input.model === "string" && input.model.trim()) {
        const value = normalizeModelOptionValue(input.model, configStore.config.channels) || input.model;
    ...
    if (input.seconds != null && String(input.seconds).trim()) {
        const seconds = clampVideoSecondsToModel(String(input.seconds), modelOptionName(finalModel));
