...
import type { WebdavSyncConfig } from "@canvas/stores/use-config-store";
import type { CanvasDeletedProject, CanvasProject } from "@canvas/stores/canvas/use-canvas-store";
...
const storageKeyPattern = /^(image|video|audio|text|file|video-reference|audio-reference):/;
