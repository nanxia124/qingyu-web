...
import { clampVideoSecondsToModel, computeVideoSize, getVideoDurationRule, inferVideoRatio, parseVideoResolution, readVideoDimensions, videoRatioOptions } from "@canvas/lib/media-size";
import { modelOptionName, type AiConfig } from "@canvas/stores/use-config-store";
...
export function VideoSettingsPanel({ config, onConfigChange, theme, showTitle = true, className = "w-[320px] space-y-4 rounded-2xl px-1 py-0.5" }: VideoSettingsPanelProps) {
    ...
    const videoModelName = modelOptionName(config.videoModel || config.model || "");
    const durationRule = getVideoDurationRule(videoModelName);
    const durationMin = durationRule.presets ? durationRule.presets[0] : durationRule.min;
    const durationMax = durationRule.presets ? durationRule.presets[durationRule.presets.length - 1] : durationRule.max;
    const seconds = Number(clampVideoSecondsToModel(config.videoSeconds || "6", videoModelName));
                ...
                <SettingGroup title={t("settingsPanels.video.seconds")} color={theme.node.muted}>
                    <div className="flex items-center gap-3" onMouseDown={(event) => event.stopPropagation()}>
                        <Slider
                            ...
                            min={durationMin}
                            max={durationMax}
                            step={durationRule.presets ? null : 1}
                            marks={durationRule.presets ? (Object.fromEntries(durationRule.presets.map((n) => [n, String(n)])) as Record<number, string>) : undefined}
                            value={seconds}
                            onChange={(value) => onConfigChange("videoSeconds", String(Array.isArray(value) ? value[0] : value))}
                        />
                        <SecondsInput value={seconds} min={durationMin} max={durationMax} modelName={videoModelName} theme={theme} onCommit={(value) => onConfigChange("videoSeconds", String(value))} />
...
function OptionPill({ selected, disabled = false, theme, onClick, children }: { selected: boolean; disabled?: boolean; theme: CanvasTheme; onClick: () => void; children: ReactNode }) {
    return (
        <button type="button" disabled={disabled} className="h-9 cursor-pointer rounded-full  px-2 text-sm transition hover:opacity-80 disabled:cursor-not-allowed disabled:opacity-35" style={{ background: "transparent", borderColor: selected ? theme.node.text : theme.node.stroke, color: theme.node.text }} onMouseDown={(event) => event.stopPropagation()} onClick={onClick}>
            {children}
        </button>
    );
}
...
function ResolutionInput({ value, theme, onChange }: { value: string; theme: CanvasTheme; onChange: (value: string) => void }) {
    return (
        <label className="flex h-9 overflow-hidden rounded-full  text-sm" style={{ color: theme.node.text }}>
...
function SecondsInput({ value, min, max, modelName, theme, onCommit }: { value: number; min: number; max: number; modelName: string; theme: CanvasTheme; onCommit: (value: number) => void }) {
    const commit = (input: HTMLInputElement) => {
        const next = Number(clampVideoSecondsToModel(input.value, modelName));
...
function SizePreview({ width, height, color }: { width: number; height: number; color: string }) {
    ...
    return <span className="rounded-[3px] -2" style={{ width: previewWidth, height: previewHeight, borderColor: color }} />;
}
