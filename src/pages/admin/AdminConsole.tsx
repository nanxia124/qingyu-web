...
import { Activity, AlertTriangle, BookOpen, Database, KeyRound, LogOut, Receipt, ShieldCheck, Users, Building2, Menu, X, ChartNoAxesCombined } from "lucide-react";
...
type Section = "api" | "api-usage" | "open-source" | "users" | "tenants" | "billing" | "audits" | "ops" | "monitor";
const nav = [
  { id: "api" as const, label: "API 与模型", icon: KeyRound, group: "核心配置" },
  { id: "api-usage" as const, label: "API 调用追踪", icon: ChartNoAxesCombined, group: "核心配置" },
...
export default function AdminConsole({ token, onLogout }: { token: string; onLogout: () => void }) {
  ...
  const content = { api: <AdminDashboard token={token} onLogout={onLogout} />, "api-usage": <AdminApiUsagePage token={token} />, "open-source": <OpenSourceDataPage />, users: <AdminUsersPage />, tenants: <AdminTenantsPage />, billing: <AdminBillingPage />, audits: <AdminAuditsPage />, ops: <AdminOpsPage />, monitor: <MonitorPage token={token} /> }[section];
  ...
  return <div className="min-h-screen bg-bg text-text">
    <header className="flex h-16 items-center justify-between bg-card px-5 shadow-sm md:px-7">
      <div className="flex items-center gap-3"><button className="rounded-lg p-2 hover:bg-secondary md:hidden" onClick={() => setMobileOpen(true)} aria-label="打开导航"><Menu size={20} /></button><div className="flex items-center gap-2"><QingyuLogoIcon className="size-8 text-text" /><img src="/litzone-wordmark.svg" alt="LitZone" className="h-9 w-auto object-contain dark:invert" /><span className="ml-1 whitespace-nowrap text-sm font-medium text-text-secondary">管理运营后台</span><EnvironmentBadge /></div></div>
    ...
    <div className="flex min-h-[calc(100vh-4rem)]">
      <aside className={`${mobileOpen ? "translate-x-0" : "-translate-x-full md:translate-x-0"} fixed inset-y-16 left-0 z-40 w-64 bg-card p-4 shadow-lg transition-transform md:static md:shadow-none`}><div className="mb-3 flex justify-end px-2 md:hidden"><button aria-label="关闭导航" onClick={() => setMobileOpen(false)}><X size={18} /></button></div>{["核心配置", "用户与运营", "系统"].map(group => <div key={group} className="mb-5"><div className="mb-2 px-2 text-[11px] font-semibold tracking-wide text-text-muted">{group}</div>{nav.filter(i => i.group === group).map(item => <button key={item.id} onClick={() => select(item.id)} className={`mb-1 flex w-full items-center gap-3 rounded-xl px-3 py-2.5 text-left text-sm transition-colors ${section === item.id ? "bg-accent-soft text-accent-soft-text" : "text-text-secondary hover:bg-secondary"}`}><item.icon size={17} />{item.label}</button>)}</div>)}</aside>
