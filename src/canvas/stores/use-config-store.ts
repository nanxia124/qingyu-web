...
// 视频模型识别关键字：在 video/sora/veo 等之外补齐豆包 seedance、Gemini omni、MiniMax h3 系列。
// 只放不会误伤图像/文本模型的“精确词”：禁止加 imagine、flash 这类同时出现在 grok-imagine-image、
// gemini-*-flash 等图像/文本模型名里的泛词，否则它们会被错判成视频、视频页默认模型落到图像模型。
const VIDEO_KEYWORDS = ["video", "sora", "veo", "kling", "wan", "hailuo", "seedance", "omni", "h3"];

export function boolConfig(value: string, fallback: boolean) {
    return value ? value === "true" : fallback;
}
const AUDIO_KEYWORDS = ["audio", "tts", "speech", "voice", "music", "sound"];
const IMAGE_KEYWORDS = ["seedream", "gpt-image", "image", "dall-e", "dalle", "imagen", "flux", "sdxl", "stable-diffusion", "midjourney", "banana", "nano-banana"];
    ...
    persist(
        ...
        }),
        {
            ...
            migrate: (persistedState: unknown, fromVersion: number) => {
                ...
                // v0 → v1：画布默认生图张数由旧默认 3 调整为 1
                if (fromVersion < 1 && state.config && (state.config as AiConfig).canvasImageCount === "3") {
                    state.config = { ...(state.config as AiConfig), canvasImageCount: "1" };
                }
                // v1 → v2：模型能力关键词扩充（新增 banana/nano-banana 等图像模型），
                // 将旧持久化模型对象降级为纯字符串，让 merge 阶段用新关键词重新猜测 capability
                if (fromVersion < 2 && state.config) {
                    const cfg = state.config as AiConfig;
                    if (Array.isArray(cfg.channels)) {
                        cfg.channels = cfg.channels.map((ch) => ({
                            ...ch,
                            models: (Array.isArray(ch.models) ? ch.models.map((m) => (typeof m === "string" ? m : m.name)) : []) as unknown as ChannelModel[],
                        }));
                    }
                }
                return state;
...
function formatModelName(model: string) {
    const special = { gpt: "GPT", grok: "Grok", dalle: "DALL-E", sd: "SD" };
    return model.split(/[-_]/).map((word) => {
        const lower = word.toLowerCase();
        if (special[lower]) return special[lower];
        if (/^\d/.test(lower)) return word;
...
export function buildApiUrl(_baseUrl: string, path: string) {
    // 走我们的服务器代理，不直接调第三方 API
