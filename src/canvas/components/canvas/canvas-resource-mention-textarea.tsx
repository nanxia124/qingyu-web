...
export const CanvasResourceMentionTextarea = forwardRef<HTMLTextAreaElement, Props>(function CanvasResourceMentionTextarea({ value, references, onChange, onSubmit, onPasteImage, onKeyDown, className, containerClassName, style, highlightLabels = true, ...props }, forwardedRef) {
    ...
    const handlePaste = (e: React.ClipboardEvent<HTMLTextAreaElement>) => {
        ...
        for (const item of Array.from(items)) {
            if (item.type.startsWith("image/")) {
                e.preventDefault();
                console.log('image found, preventing default');
                const blob = item.getAsFile();
                if (blob && onPasteImage) {
                    const file = new File([blob], "clipboard-image.png", { type: item.type });
                    onPasteImage(file);
                }
                return;
            }
...
function MentionMenu({ textarea, caretIndex, references, activeIndex, theme, onSelect }: { textarea: HTMLTextAreaElement; caretIndex: number; references: CanvasResourceReference[]; activeIndex: number; theme: (typeof canvasThemes)[keyof typeof canvasThemes]; onSelect: (reference: CanvasResourceReference) => void }) {
    ...
    return createPortal(
        <div
            ...
            className="fixed z-[120] max-h-56 w-64 overflow-y-auto rounded-xl border p-1 backdrop-blur-md canvas-float"
