    ...
    return (
        <div className="space-y-3">
            <section className="rounded-lg border border-zinc-200 p-4 dark:border-zinc-800">
                <div className="flex flex-wrap items-start justify-between gap-3">
                    <div>
                        ...
                        <div className="mt-1 text-xs text-zinc-500">{t("config.localStorage.description")}</div>
                    </div>
            ...
            {usage?.databases.map((database) => (
                <section key={database.name} className="overflow-hidden rounded-lg border border-zinc-200 dark:border-zinc-800">
                    <div className="flex items-center justify-between gap-3 border-b border-zinc-200 px-4 py-3 dark:border-zinc-800">
                        <div className="min-w-0">
                            <div className="truncate text-sm font-semibold">{t("config.localStorage.mainDatabase")}</div>
                            <div className="mt-0.5 truncate font-mono text-[11px] text-zinc-500">{database.name} · v{database.version}</div>
                        </div>
                        ...
                        {database.stores.map((store) => (
                            <div key={store.name} className="grid grid-cols-[minmax(0,1fr)_auto_auto] items-center gap-4 px-4 py-3 text-sm">
                                <div className="min-w-0">
                                    ...
                                    <div className="mt-0.5 truncate font-mono text-[11px] text-zinc-500">{store.name}</div>
                                </div>
                                <div className="text-right text-xs text-zinc-500 tabular-nums">{t("config.localStorage.records", { count: store.records })}</div>
...
function StorageMetric({ icon, label, value, hint }: { icon: ReactNode; label: string; value: string; hint: string }) {
    return (
        <div className="rounded-lg bg-zinc-100/70 p-3 dark:bg-zinc-900/70">
