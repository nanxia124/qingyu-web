...
export default function AdminBillingPage() {
  ...
  return (
    <div className="p-6 max-w-6xl mx-auto">
      <h1 className="text-xl font-bold text-text mb-4">计费管理</h1>
        ...
        <div className="rounded-xl bg-card border border-border overflow-hidden">
          <table className="w-full text-sm">
            <thead className="bg-secondary">
              <tr>{["邮箱", "余额", "会员", "到期", "累计消费", "操作"].map(h => <th key={h} className="text-left px-4 py-3 text-gray-500">{h}</th>)}</tr>
            </thead>
            <tbody>
              {users.map(u => (
                <tr key={u.id} className="border-t border-border">
                  ...
                  <td className="px-4 py-3">
                    <button onClick={() => adjustUser(u)} className="text-accent hover:underline text-xs">调整积分</button>
                  </td>
                </tr>
              ))}
            </tbody>
      ...
      {tab === "orders" && (
        <div className="rounded-xl bg-card border border-border overflow-hidden">
          <table className="w-full text-sm">
            <thead className="bg-secondary">
              <tr>{["订单号", "用户", "套餐", "金额", "状态", "时间", "操作"].map(h => <th key={h} className="text-left px-4 py-3 text-gray-500">{h}</th>)}</tr>
            </thead>
            <tbody>
              {orders.map(o => (
                <tr key={o.id} className="border-t border-border">
                  ...
                  <td className="px-4 py-3">
                    {o.status !== "paid" && (
                      <button onClick={() => completeOrder(o)} className="text-accent hover:underline text-xs">标记已付</button>
                    )}
                  </td>
                </tr>
              ))}
      ...
      {tab === "codes" && (
        <div className="space-y-4">
          <div className="rounded-xl bg-card border border-border p-4 flex flex-wrap items-end gap-3">
            <div><label className="block text-xs text-gray-500 mb-1">数量</label><input type="number" value={genForm.count} onChange={e => setGenForm({...genForm, count: +e.target.value})} className="w-20 rounded bg-secondary border border-border px-2 py-1.5 text-text" /></div>
            ...
            {genForm.kind === "quota" && (
              <div><label className="block text-xs text-gray-500 mb-1">面额(积分)</label><input type="number" value={genForm.denomination} onChange={e => setGenForm({...genForm, denomination: +e.target.value})} className="w-24 rounded bg-secondary border border-border px-2 py-1.5 text-text" /></div>
            )}
            <div><label className="block text-xs text-gray-500 mb-1">有效期(天,0永久)</label><input type="number" value={genForm.days} onChange={e => setGenForm({...genForm, days: +e.target.value})} className="w-24 rounded bg-secondary border border-border px-2 py-1.5 text-text" /></div>
            <button onClick={genCodes} className="px-4 py-2 rounded-lg bg-[#5051F8] text-white hover:bg-accent-hover">生成兑换码</button>
          </div>
          {genResult.length > 0 && (
            <div className="rounded-xl bg-card border border-border p-4">
              <div className="text-sm text-gray-500 mb-2">本次生成（可复制导出）：</div>
          ...
          <div className="rounded-xl bg-card border border-border overflow-hidden">
            <table className="w-full text-sm">
              <thead className="bg-secondary"><tr>{["兑换码", "面额", "状态", "使用者", "生成时间"].map(h => <th key={h} className="text-left px-4 py-3 text-gray-500">{h}</th>)}</tr></thead>
              <tbody>
                {codes.map(c => (
                  <tr key={c.code} className="border-t border-border">
                    ...
                    <td className="px-4 py-3">{c.usedBy ? <span className="text-xs text-gray-500">已用</span> : <span className="text-xs text-green-600">未用</span>}</td>
      ...
      {tab === "plans" && (
        <div className="rounded-xl bg-card border border-border p-4 text-gray-500 text-sm">
          套餐价格与额度配置请直接编辑 <code className="text-green-600">api-data/billing_plans.json</code>，或后续接入可视化编辑。当前包含 free / pro / team 三档。
        </div>
      )}
    </div>
  );
