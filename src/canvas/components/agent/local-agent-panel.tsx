...
import { readAgentUrlBootstrap } from "@canvas/lib/agent/agent-url-bootstrap";
...
import { resolveCanvasReferenceImages } from "@canvas/lib/canvas/canvas-resource-references";
...
export function LocalAgentPanel({ embedded, headless, autoConnect }: { embedded?: boolean; headless?: boolean; autoConnect?: boolean }) {
    ...
    const toggleAgentConnection = async ({ silent = false }: { silent?: boolean } = {}) => {
        ...
        errorLoggedRef.current = false;
        setAgentState({ url: nextEndpoint, token: nextToken, enabled: true, connected: false, silentConnect: silent, fragmentBootstrap: false, activity: rt("connecting"), connectError: "", activeTab: "chat" });
    };
    ...
    useLayoutEffect(() => {
        ...
        errorLoggedRef.current = false;
        setAgentState({ url: bootstrap.url.replace(/\/$/, ""), token: bootstrap.token, enabled: true, connected: false, silentConnect: true, fragmentBootstrap: true, confirmTools: false, activity: rt("connecting"), connectError: "", activeTab: "chat" });
                    ...
                    {!connected ? (
                        <div className="flex flex-1 flex-col gap-3 overflow-hidden p-4">
                            <div className="flex items-center gap-2">
                                <div className="size-8 animate-pulse rounded-full" style={{ background: theme.node.stroke }} />
                                <div className="h-3 w-16 animate-pulse rounded" style={{ background: theme.node.stroke }} />
                            </div>
                            <div className="ml-10 h-3 w-3/4 animate-pulse rounded" style={{ background: theme.node.stroke }} />
                            <div className="ml-10 h-3 w-1/2 animate-pulse rounded" style={{ background: theme.node.stroke }} />
                            <div className="mt-2 flex justify-end">
                                <div className="h-8 w-1/3 animate-pulse rounded-2xl" style={{ background: theme.node.stroke }} />
                            </div>
                            <div className="ml-10 h-3 w-2/3 animate-pulse rounded" style={{ background: theme.node.stroke }} />
