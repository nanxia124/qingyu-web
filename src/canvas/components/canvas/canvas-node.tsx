...
import { pickImageSource } from "@canvas/lib/image-thumbnail";
import { previewUrlFor, subscribeImagePreviews, getImagePreviewRevision } from "@canvas/services/image-storage";
...
export const CanvasNode = React.memo(function CanvasNode({
    ...
    // 图片节点标题栏右侧的分辨率：优先用成品图实际尺寸，未生成时回退到配置尺寸
    const resolutionLabel = useMemo(() => {
        if (data.type !== CanvasNodeType.Image) return null;
        const imgs = data.metadata?.images;
        const primary = imgs?.find((img) => img.id === (data.metadata?.primaryImageId || imgs[0]?.id));
        const w = data.metadata?.naturalWidth || primary?.naturalWidth;
        const h = data.metadata?.naturalHeight || primary?.naturalHeight;
        if (w && h) return `${w}×${h}`;
        const size = data.metadata?.size;
        if (size && /\d{3,4}\s*[x×]\s*\d{3,4}/.test(size)) return size.replace(/\s*[x×]\s*/, "×");
        return null;
    }, [data]);
    ...
    return (
        <div
            ...
            className={`node-element group/node absolute flex select-none flex-col transition-shadow duration-200 ${isGroup ? "z-[5]" : isSelected ? "z-50" : "z-10"} ${referenceSelectionState === "available" ? "cursor-pointer" : referenceSelectionState ? "cursor-not-allowed" : isSelected ? "cursor-grab" : ""}`}
            style={{
        ...
        >
            {!referenceSelectionState && (isSelected || hovered || isEditingTitle) && (
                <div className="absolute left-3 top-[-28px] z-[65] flex items-center gap-2" style={{ width: Math.max(0, data.width - 24) }} onMouseDown={(event) => event.stopPropagation()} onPointerDown={(event) => event.stopPropagation()}>
                    {isEditingTitle ? (
                        <input
                            ref={titleInputRef}
                            value={titleDraft}
                        ...
                        <button
                            type="button"
                            className="block min-w-0 flex-1 truncate border-b border-dashed border-transparent px-0 py-0.5 text-left text-xs font-medium opacity-75 transition hover:border-current hover:opacity-100"
                    ...
                    {resolutionLabel ? (
                        <span className="shrink-0 select-none py-0.5 text-[10px] font-normal tabular-nums opacity-60" style={{ color: theme.node.muted }}>
                            {resolutionLabel}
                        </span>
                    ) : null}
                </div>
            ...
            {showPanel && !isGroup && renderPanel ? <div className="absolute left-1/2 top-full z-[70] w-[600px] -translate-x-1/2 pt-4" onMouseDown={(event) => { if (!referenceSelectionState) onMouseDown(event, data.id); }}>{renderPanel(data)}</div> : null}
        </div>
    );
});
    ...
    return (
        <BatchFrame batchCount={batchCount} batchExpanded={batchExpanded}>
            ...
            <div className="flex h-full w-full flex-col overflow-hidden rounded-md">
                {isEditingContent ? (
                    <CanvasResourceMentionTextarea
                        ...
                        onPasteImage={(file) => {
                            // 粘贴图片时触发全局事件
                            window.dispatchEvent(new CustomEvent('canvas-paste-image', { detail: file }));
                        }}
                    />
                ) : content ? (
            ...
            {isBatchRoot ? (
                <button
                    type="button"
                    className="absolute right-2.5 top-2.5 z-30 flex h-8 items-center justify-center gap-1.5 rounded-full border px-3 text-xs font-semibold shadow-[0_6px_18px_rgba(0,0,0,.12)] backdrop-blur-md transition hover:scale-[1.02]"
    ...
    return (
        <BatchFrame batchCount={batchCount} batchExpanded={batchExpanded}>
            ...
            {isBatchRoot ? (
                <button
                    type="button"
                    className="absolute right-2.5 top-2.5 z-30 flex h-8 items-center justify-center gap-1.5 rounded-full border px-3 text-xs font-semibold shadow-[0_6px_18px_rgba(0,0,0,.16)] backdrop-blur-md transition hover:scale-[1.02]"
                    style={{ background: theme.toolbar.panel, borderColor: theme.toolbar.border, color: theme.toolbar.activeText }}
...
function BatchImageFailureActions({ placement, onRetry, onDelete }: { placement: "left" | "right"; onRetry: () => void; onDelete: () => void }) {
    ...
    return (
        <div className={`absolute top-3 z-30 flex items-center gap-1.5 ${placement === "left" ? "left-3" : "right-3"}`}>
            ...
            <button type="button" className="grid size-8 place-items-center rounded-lg border transition hover:scale-[1.02]" style={{ background: theme.toolbar.panel, borderColor: theme.toolbar.border, color: theme.node.text }} onClick={(event) => (event.stopPropagation(), onDelete())} aria-label={t("common.delete")}>
            ...
            {isBatchRoot ? (
                <div className="pointer-events-none absolute inset-0 overflow-visible">
                    {Array.from({ length: Math.min(batchCount - 1, 3) }).map((_, index) => (
                        <div
                            key={index}
                            className="absolute rounded-[inherit] border shadow-[0_10px_24px_rgba(0,0,0,.12)] transition-all duration-300 group-hover/batch:translate-x-1"
                            style={{
                                inset: 0,
