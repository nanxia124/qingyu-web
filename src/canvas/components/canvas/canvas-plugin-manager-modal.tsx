...
import { installPluginFromUrl, setPluginEnabled, uninstallPlugin, updatePlugin } from "@canvas/lib/canvas/plugin-loader";
import { fetchOfficialPlugins, hasUpgrade, type OfficialPluginEntry } from "@canvas/lib/canvas/plugin-registry";
import { useThemeStore } from "@canvas/stores/use-theme-store";
import { usePluginStore, type InstalledPlugin } from "@canvas/stores/canvas/use-plugin-store";
...
export function CanvasPluginManagerModal({ open, onClose }: { open: boolean; onClose: () => void }) {
    ...
    return (
        <Modal title={t("canvas.plugins.title")} open={open} onCancel={onClose} footer={null} centered width={640}>
            <div className="space-y-3">
                <div className="flex items-start gap-2 rounded-lg border px-3 py-2 text-xs leading-5" style={{ borderColor: "#fbbf2455", background: "#fbbf2414", color: theme.node.text }}>
