...
import { listNodeDefinitions, useNodeRegistryVersion } from "@canvas/lib/canvas/node-registry";
import { CanvasNodeType, type ConnectionHandle, type Position } from "@canvas/types/canvas";
...
export function ConnectionCreateMenu({
    ...
    return (
        <div
            className="absolute z-[120] w-[300px] rounded-[18px] border p-3 backdrop-blur canvas-float"
...
export function NodeCreateMenu({ position, onCreate, onClose }: { position: Position; onCreate: (type: string) => void; onClose: () => void }) {
    ...
    return (
        <div
            ref={menuRef}
            className="absolute z-[120] max-h-[70vh] w-[300px] overflow-y-auto rounded-[18px] border p-3 backdrop-blur thin-scrollbar canvas-float"
            data-canvas-no-zoom
