...
function initSherpaOnnxOfflinePunctuationModelConfig(config, Module) {
  const modelPath = config.ctTransformer || '';
  const provider = config.provider || 'cpu';
  const modelPathLen = Module.lengthBytesUTF8(modelPath) + 1;
  const providerLen = Module.lengthBytesUTF8(provider) + 1;

  const buffer = Module._malloc(modelPathLen + providerLen);
  Module.stringToUTF8(modelPath, buffer, modelPathLen);
  Module.stringToUTF8(provider, buffer + modelPathLen, providerLen);
  ...
  Module.setValue(ptr + offset, buffer + modelPathLen, 'i8*');
...
function initSherpaOnnxOfflinePunctuationConfig(config, Module) {
  const model = initSherpaOnnxOfflinePunctuationModelConfig(
      config.model || {}, Module);
...
class OfflinePunctuation {
  ...
  addPunctuation(text) {
    ...
    if (!this.handle) {
      throw new Error('Offline punctuation instance has been destroyed');
    }
    const textLen = this.Module.lengthBytesUTF8(text) + 1;
    const buffer = this.Module._malloc(textLen);
    this.Module.stringToUTF8(text, buffer, textLen);
    const resultPtr = this.Module._SherpaOfflinePunctuationAddPunct(
        this.handle, buffer);
    this.Module._free(buffer);
    const punctuated = this.Module.UTF8ToString(resultPtr);
    this.Module._SherpaOfflinePunctuationFreeText(resultPtr);
    return punctuated;
  }
};
