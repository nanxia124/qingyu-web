...
type GetCanvasCenter = () => { x: number; y: number };
...
export function useAgentBridge(params: AgentBridgeParams) {
    const { projectId, title, nodes, connections, selectedNodeIds, viewport, nodesRef, connectionsRef, selectedNodeIdsRef, viewportRef, generateNodeRef, getCanvasCenter, setNodes, setConnections, setSelectedNodeIds, setSelectedConnectionId, setViewport, setContextMenu } =
        params;
    ...
    const applyAgentOps = useCallback(
        (ops?: CanvasAgentOp[]) => {
            const safeOps = (Array.isArray(ops) ? ops.filter((op) => op?.type) : []).map((op) => {
                // agent 创建节点但未指定位置时，自动放到当前画布视口中心，多个节点错开排列
                if (op.type === "add_node" && !op.position && op.x === undefined && op.y === undefined && getCanvasCenter) {
                    const center = getCanvasCenter();
                    const addNodeOps = (Array.isArray(ops) ? ops : []).filter((o) => o?.type === "add_node");
                    const index = addNodeOps.indexOf(op);
                    const offset = index * 40;
                    return { ...op, position: { x: center.x - 120 + offset, y: center.y - 120 + offset } };
                }
                return op;
            });
