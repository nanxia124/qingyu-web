...
import { ConfigLocalProxy } from "@canvas/components/layout/config-local-proxy";
import { ConfigPromptSources } from "@canvas/components/layout/config-prompt-sources";
    ...
    return (
        <>
            <div className="flex flex-wrap items-center justify-between gap-3 border-b border-zinc-200 pb-3 dark:border-zinc-800">
                <div className="text-xs text-zinc-500">{t("config.fileSecurity")}</div>
                        ...
                        children: (
                            <div>
                                <div className="mb-4 flex flex-wrap items-center justify-between gap-3">
                                    <div className="text-xs text-zinc-500">{t("config.channels.description")}</div>
                                ...
                                <div className="space-y-2">
                                    {config.channels.map((channel) => (
                                        <div key={channel.id} className="flex items-center justify-between gap-3 rounded-lg border border-zinc-200 px-4 py-3 dark:border-zinc-800">
                                ...
                                <section className="rounded-lg border border-zinc-200 p-3 dark:border-zinc-800">
                                    <div className="mb-3 flex flex-wrap items-start justify-between gap-3">
                                        <div>
                                            ...
                                            <div className="mt-1 text-xs text-zinc-500">{t("config.webdav.description")}</div>
                                        </div>
                                        <div className="text-xs text-zinc-500">{webdav.lastSyncedAt ? t("config.webdav.lastSynced", { time: formatWebdavTime(webdav.lastSyncedAt, locale) }) : t("config.webdav.neverSynced")}</div>
                                    </div>
                                    ...
                                    <div className="mt-4 flex flex-wrap items-center gap-2">
                                        ...
                                        {webdavSyncStatus ? <span className="text-xs text-zinc-500">{syncStageLabel(webdavSyncStatus, t)}</span> : null}
                                    </div>
        ...
        <Modal
            title={
                <div>
                    ...
                    <div className="mt-1 text-xs font-normal text-zinc-500">{t("config.modalDescription")}</div>
                </div>
            }
            open={isConfigOpen}
            width={980}
    ...
    return (
        <div className="mt-3 grid gap-2">
            {webdavDomainKeys.map((key) => {
                ...
                return (
                    <div key={key} className="rounded-md border border-zinc-200 px-3 py-2 dark:border-zinc-800">
                        <div className="mb-1 flex min-w-0 items-center justify-between gap-3 text-xs">
                            <span className="shrink-0 font-medium text-zinc-700 dark:text-zinc-200">{t(`config.webdav.domains.${domainTranslationKey(key)}`)}</span>
