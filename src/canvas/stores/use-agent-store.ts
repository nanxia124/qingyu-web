...
export const useAgentStore = create<AgentStore>((set, get) => ({
    ...
    panelClosing: false,
    canvasContext: null,
    url: typeof window === "undefined" ? import.meta.env.VITE_AGENT_URL || "http://127.0.0.1:17371" : localStorage.getItem("canvas-agent-url") || import.meta.env.VITE_AGENT_URL || "http://127.0.0.1:17371",
    ...
    closePanel: () => {
        ...
        set({ panelOpen: true, panelClosing: true });
        setTimeout(() => {
    ...
    addMessage: (item) => set((state) => ({ messages: [...state.messages, { timestamp: Date.now(), ...item }] })),
