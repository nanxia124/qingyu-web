...
import { fetchPromptSourceStatuses, refreshAllSources, refreshSource } from "@canvas/services/api/prompts";
import { PROMPT_SOURCE_INTERVALS, usePromptSourceStore } from "@canvas/stores/use-prompt-source-store";
        ...
        <div>
            ...
            <div className="space-y-2">
                {sources.map((source) => {
                    ...
                    return (
                        <div key={source.id} className="flex flex-wrap items-center gap-3 rounded-lg border border-zinc-200 px-4 py-3 dark:border-zinc-800">
                            ...
                            <div className="min-w-[220px] flex-1">
                                ...
                                <div className="mt-1 flex min-w-0 flex-wrap items-center gap-x-3 gap-y-1 text-xs text-zinc-500">
                                    <a className="max-w-full truncate hover:text-zinc-800 hover:underline dark:hover:text-zinc-200" href={source.homepage || source.url} target="_blank" rel="noreferrer">
            ...
            <section className="mt-5 rounded-lg border border-zinc-200 p-4 dark:border-zinc-800">
                ...
                <div className="flex flex-wrap items-center gap-3">
                    <div className="flex items-center gap-2">
                        <span className="text-xs text-zinc-500">{t("config.promptSources.interval")}</span>
                    ...
                    <span className="text-xs text-zinc-500">{schedule.lastFetchedAt ? t("config.promptSources.lastFetched", { time: formatTime(schedule.lastFetchedAt, i18n.resolvedLanguage) }) : t("config.promptSources.neverScheduled")}</span>
                </div>
                <div className="mt-2 text-xs text-zinc-400">{t("config.promptSources.scheduleDescription")}</div>
            </section>
