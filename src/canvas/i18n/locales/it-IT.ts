// Auto-generated partial translation: main-site strings. Canvas editor falls back to en-US.
import enUS from "@canvas/i18n/locales/en-US";
function dm(a,b){const o={...a};for(const k in b){if(b[k]&&typeof b[k]==="object"&&!Array.isArray(b[k]))o[k]=dm(a[k]||{},b[k]);else o[k]=b[k];}return o;}
const override={
    simpleChat: {
        newConversation: "Nuova conversazione",
        historyEmpty: "Nessuna conversazione",
        newConversationFallback: "Nuova conversazione",
        exportChat: "Esporta conversazione",
        chatSettings: "Impostazioni chat",
        enterToSend: "Invio per inviare",
        autoScroll: "Scorri automaticamente ai nuovi messaggi",
        clearAfterSend: "Cancella input dopo l'invio",
        enterToSendHint: "Invio per inviare · Maiusc+Invio per andare a capo",
        ctrlEnterToSendHint: "Ctrl/Cmd+Invio per inviare · Invio per andare a capo",
        settingsSavedHint: "Le impostazioni vengono salvate in questo browser",
        heroTitle: "Come posso aiutarti?",
        heroSubtitle: "Scrivi la tua domanda per iniziare la conversazione",
        thinking: "Sto pensando...",
        retry: "Riprova",
        sendFailed: "Connessione interrotta. Controlla la rete e riprova.",
        sendFailedToast: "Errore di rete, riprova più tardi",
        aborted: "Generazione interrotta",
        speak: "Leggi ad alta voce",
        stopSpeak: "Interrompi lettura",
        regenerate: "Rigenera",
        copy: "Copia",
        copied: "Copiato",
        copyFailed: "Copia non riuscita",
        tokensTooltip: "Input {{in}} · Output {{out}}",
        webSearch: "Ricerca web",
        inputPlaceholder: "Scrivi la tua domanda...",
        stop: "Interrompi",
        send: "Invia",
        deleteTitle: "Elimina conversazione",
        deleteConfirm: "Sei sicuro di voler eliminare questa conversazione? Questa azione non può essere annullata.",
        delete: "Elimina",
        cancel: "Annulla",
        deleted: "Eliminata",
        deleteFailed: "Eliminazione non riuscita",
        createFailed: "Creazione conversazione non riuscita",
        exportEmpty: "Nessun contenuto da esportare",
        exported: "Esportato",
        ttsUnsupported: "La sintesi vocale non è supportata da questo browser",
        today: "Oggi",
        yesterday: "Ieri",
        roleDefault: "Predefinito",
        roleCoder: "Assistente di codice",
        roleWriter: "Compagno di scrittura",
        roleTranslator: "Traduttore",
        roleInterviewer: "Colloquio simulato",
        markdownMe: "Io",
        markdownAI: "IA",
        defaultTitle: "Conversazione",
    },

    apiErrors: {
      contentRejected: "Contenuto rifiutato dalle linee guida di sicurezza, modifica il prompt",
      imageRequestFailed: "Modifica fallita, riprova",
    },
    topNav: {
      switchLanguage: "Passa a {{language}}",
      shortcuts: "Scorciatoie da tastiera",
      lightTheme: "Tema chiaro",
      darkTheme: "Tema scuro",
    },
    home: {
      start: "Inizia",
    },
    videoWorkbench: {
      title: "Studio video",
      promptPlaceholder: "Descrivi movimento, atmosfera…",
      references: "Immagini di riferimento",
      videoReferences: "Video di riferimento",
      model: "Modello",
      ratio: "Proporzione",
      quality: "Qualità",
      size: "Dimensione",
      duration: "Durata",
      mode: "Modalità",
      clear: "Pulisci",
      upload: "Carica",
      historyTitle: "Le mie generazioni",
      historyEmpty: "Nessuna generazione ancora",
      historyLoading: "Caricamento…",
      statusRunning: "Generazione",
      statusSaving: "Salvataggio del risultato…",
      statusDone: "Completato",
      statusFailed: "Fallito",
      statusRefunded: "Rimborsato",
      submitFailed: "Invio fallito, riprova più tardi",
      taskQueryFailed: "Richiesta fallita, riprova",
      historyQueryFailed: "Impossibile caricare la cronologia, riprova",
      pollTimeout: "Il risultato sta impiegando un po' di tempo; apparirà più tardi nella cronologia",
      toasts: {
        history: "Le mie generazioni",
        timeoutRefunded: "Generazione scaduta, crediti rimborsati",
        failedRefunded: "Generazione fallita, crediti rimborsati",
      },
      generate: "Genera",
      results: "Risultati",
      download: "Scarica",
      saveAsset: "Salva nelle risorse",
    },
    canvas: {
      editors: {
              reset: "Reimposta",
              cancel: "Annulla",
              confirmCrop: "Ritaglia",
              cropTitle: "Ritaglia immagine",
              maskTitle: "Modifica locale",
              maskGenerate: "Genera ora",
              splitTitle: "Dividi immagine",
              upscaleTitle: "Migliora immagine",
              rows: "Righe",
              columns: "Colonne",
              brush: "Pennello",
              erase: "Gomma",
              brushSize: "Dimensione",
            },
            plugins: {
              title: "Plugin",
              install: "Installa",
              uninstall: "Disinstalla",
              refresh: "Aggiorna",
              installed: "Installato",
              enabled: "Abilitato",
              disabled: "Disabilitato",
            },
      sidePanel: {
              canvas: "Tavola",
              assets: "Risorse",
              prompts: "Libreria prompt",
              elements: "Elementi della tavola",
              searchNodes: "Cerca nodi",
              noNodes: "Nessun nodo",
              clearAll: "Pulisci tutto",
              searchAssets: "Cerca risorse",
              noAssets: "Nessuna risorsa",
            },
            imageTools: {
              copyPrompt: "Copia prompt",
              replace: "Sostituisci immagine",
              crop: "Ritaglia",
              upscale: "Migliora risoluzione",
              view: "Vedi immagine",
              more: "Altro",
            },
            nodeToolbar: {
              info: "Info",
              removeTitle: "Rimuovi nodo",
              downloadImage: "Scarica immagine",
              downloadVideo: "Scarica video",
              downloadAudio: "Scarica audio",
              editText: "Modifica testo",
              saveAsset: "Salva nelle risorse",
            },
            configNode: {
              title: "Configurazione",
              prompt: "Prompt",
              generate: "Genera",
              stop: "Ferma",
            },
            promptPanel: {
              generate: "Genera",
              stop: "Ferma",
              image: "Descrivi l'immagine…",
              video: "Descrivi il video…",
            },
            controls: {
              ratio: "Proporzione",
              duplicate: "Duplica",
              delete: "Elimina",
            },
      library: "Tavole",
      projects: "Le mie tele",
      create: "Nuova tela",
      empty: "Nessuna tela",
      undo: "Annulla",
      redo: "Ripeti",
      resetView: "Reimposta vista",
      home: "Home",
      docs: "Documentazione",
      toolbar: {
        select: "Seleziona",
        pan: "Sposta",
        upload: "Carica risorse",
        clear: "Svuota tela",
        themeMode: "Tema",
        light: "Chiaro",
        dark: "Scuro",
      },
      project: {
        rename: "Rinomina",
        delete: "Elimina",
        export: "Esporta",
      },
      createMenu: {
        close: "Chiudi",
      },
      node: {
        untitled: "Nodo senza titolo",
        generate: "Genera",
        retry: "Riprova",
        renameHint: "Doppio clic per rinominare",
      },
    },
    workbench: {
      logs: "Cronologia",
      settings: "Impostazioni",
      prompt: "Prompt",
      viewPrompts: "Sfoglia prompt",
      clipboard: "Appunti",
      upload: "Carica",
      generate: "Genera",
      results: "Risultati",
      generating: "Generazione…",
      success: "Riuscito",
      failed: "Fallito",
      retry: "Riprova",
      new: "Nuovo",
      selectAll: "Seleziona tutto",
      noLogs: "Nessuna cronologia",
    },
    imageWorkbench: {
      title: "Studio immagini",
      promptPlaceholder: "Descrivi soggetto, stile, luce…",
      references: "Immagini di riferimento",
      dropReferences: "Rilascia per aggiungere",
      noReferences: "Nessuna immagine di riferimento",
      promptRequired: "Inserisci un prompt",
    },
    assets: {
      title: "Mie risorse",
      fields: {
        title: "Titolo",
        tags: "Tag",
        source: "Fonte",
        note: "Nota",
      },
      preview: "Anteprima",
      details: "Dettagli",
      deleteTitle: "Elimina risorsa",
      copyText: "Copia testo",
      downloadImage: "Scarica immagine",
    },
    settingsPanels: {
      common: {
        auto: "Auto",
        low: "Bassa",
        medium: "Media",
        high: "Alta",
      },
      image: {
        title: "Impostazioni immagine",
        quality: "Qualità",
        size: "Dimensione",
        count: "Quantità",
      },
      video: {
        title: "Impostazioni video",
        seconds: "Secondi",
        duration: "Durata",
      },
      model: {
        select: "Scegli modello",
        refresh: "Aggiorna",
      },
    },
    prompts: {
      title: "Centro prompt",
      library: "Libreria",
      category: "Categoria",
      tags: "Tag",
      search: "Cerca",
      use: "Usa",
      empty: "Nessun risultato",
      loading: "Caricamento…",
    },
    common: {
      cancel: "Annulla",
      save: "Salva",
      delete: "Elimina",
      copy: "Copia",
      done: "Fatto",
      upload: "Carica",
      download: "Scarica",
    },
    nav: {
      home: "Home",
      chat: "Chat",
      image: "Immagine",
      video: "Video",
      canvas: "Tela",
      assets: "Risorse",
      favorites: "Preferiti",
      teams: "Team",
      my: "Account",
      darkMode: "Scuro",
      lightMode: "Chiaro",
      language: "Lingua",
    },
    mainHome: {
      searchPlaceholder: "Cerca…",
      invite: "Invita e guadagna",
      points: "Punti",
      vip: "Abbonamento",
      soon: "Prossimamente",
      text2img: "Testo in immagine",
      text2imgDesc: "Da testo a immagine",
      expand: "Espandi",
      expandDesc: "Estensione automatica",
      style: "Stili",
      styleDesc: "Restyle con un clic",
      works: "Le mie opere",
      worksDesc: "Gestisci cronologia",
      catAll: "Tutti",
      catEcommerce: "E-commerce",
      catPoster: "Poster",
      catPhoto: "Foto",
      catIllustration: "Illustrazione",
      recommend: "Consigliati",
      latest: "Ultimi",
      empty: "Nessuna opera trovata",
      inputPlaceholder: "Descrivi cosa vuoi creare…",
    },
    auth: {
      subtitleLogin: "Accedi o registrati",
      useGoogle: "Continua con Google",
      useApple: "Continua con Apple",
      or: "o",
      email: "Email",
      nickname: "Nickname",
      password: "Password (min. 6)",
      forgot: "Password dimenticata?",
      continue: "Continua",
      login: "Accedi",
      register: "Registrati",
    },
    chat: {
      greeting: "Come posso aiutarti?",
      send: "Invia",
      selectModel: "Scegli modello",
    },
    pages: {
      settings: {
        api: "API",
        notify: "Notifiche",
        security: "Sicurezza",
        appearance: "Aspetto",
        team: "Team",
        data: "Dati",
        serverAddr: "URL server",
        save: "Salva",
        developing: "Prossimamente",
      },
      subscription: {
        title: "Piani",
        free: "Gratis",
        perMonth: "/mese",
      },
      wallet: {
        title: "Il mio portafoglio",
        currentBalance: "Saldo",
      },
      feedback: {
        title: "Feedback",
        submit: "Invia",
      },
      favorites: {
        title: "Preferiti",
      },
      assets: {
        all: "Tutti",
        image: "Immagini",
        video: "Video",
        upload: "Carica",
      },
    },
};
export default dm(enUS,override);
